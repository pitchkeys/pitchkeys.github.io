...
class SongFull extends Component {
    ...
    componentDidMount(){
        ...
        if(window.location.href.charAt(window.location.href.length-12) == "/"){
            fetch ('https://pitchkeys-api.herokuapp.com/api/songs/' + window.location.href.substring(window.location.href.length-11)).then(res => res.text()).then(data => {
    ...
    updateDownloadCount(e){
        ...
        fetch("https://pitchkeys-api.herokuapp.com/api/songs/"+ window.location.href.substring(window.location.href.length-7), {
            method: 'POST',
    ...
    submitRating(e){
        ...
        if(this.state.reviewValue < 6 && this.state.reviewValue > 0 && this.state.reviewValue.length > 0){
            ...
            let d = {stats: newObject}
            fetch("https://pitchkeys-api.herokuapp.com/api/songs/"+ window.location.href.substring(window.location.href.length-7) + "/add", {
            method: 'POST',
            body: JSON.stringify(d),
            headers: {
                'Content-Type': 'application/json'
